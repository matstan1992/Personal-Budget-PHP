<?php
	...
	if (isset($_POST['date1'])) {
		...
		if ($dateStart == NULL) {
			$allGood = false;
			$_SESSION['e_dateStart'] = "Wybierz datę początku okresu";
			header('Location: balance.php');
		}
				
		if ($dateEnd == NULL) {
			$allGood = false;
			$_SESSION['e_dateEnd'] = "Wybierz datę końca okresu";
			header('Location: balance.php');
		}				
					
		if ($dateStart->format('Y-m-d') > $lastDayOfTheMonth) {
			$allGood = false;
			$_SESSION['e_dateStart'] = "Data początku okresu nie może przekraczać daty ostatniego dnia bieżącego miesiąca";
			header('Location: balance.php');
		}
					
		if ($dateEnd->format('Y-m-d') > $lastDayOfTheMonth) {
			$allGood = false;
			$_SESSION['e_dateEnd'] = "Data końca okresu nie może przekraczać daty ostatniego dnia bieżącego miesiąca";
			header('Location: balance.php');
		}
		
		if ($dateStart->format('Y-m-d') < '2000-01-01') {
			$allGood = false;
			$_SESSION['e_dateStart'] = "Minimalna data początku okresu to 01-01-2000";
			header('Location: balance.php');
		}	
		
		if ($dateEnd->format('Y-m-d') < '2000-01-01') {
			$allGood = false;
			$_SESSION['e_dateEnd'] = "Minimalna data końca okresu to 01-01-2000";
			header('Location: balance.php');
		}
		...
		if ($dateEnd!=NULL && $dateStart!=NULL) {
			if($dateEnd < $dateStart) {
				$allGood = false;
				$_SESSION['e_dateEnd'] = "Data końca okresu nie może być wcześniejsza od daty początku okresu";
				header('Location: balance.php');
			}
		}
